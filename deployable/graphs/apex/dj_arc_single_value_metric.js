...
.col {
  ...
  updateAsync: function(
    ...
    if (data[0][queryResponse.fields.measure_like[0].name].rendered)
      value = data[0][queryResponse.fields.measure_like[0].name].rendered;
    ...
    if (document.getElementById("apex-chart")) columnOrBarChart.render();
    ...
    function sparkLines() {
      ...
      if (sparkline == `barWithLabels`) {
        ...
        if (config.fbcStrokeWidth)
          strokeWidth = parseInt(config.fbcStrokeWidth, 10);
      ...
      if (sparkline == `radial`) {
        ...
        if (config.rcDonutHoleMargin)
          donutHoleMargin = parseInt(config.rcDonutHoleMargin, 10);
      ...
      if (sparkline == `area`) {
        ...
        if (config.areaMarkerSize)
          markerSize = parseInt(config.areaMarkerSize, 10);
      ...
      if (sparkline == `multi`) {
        ...
        if (seriesCount != seriesInformation.seriesData.length) {
          for (let i = 0; i < seriesCount; i++)
            delete settings[`seriesType_${i}`];
          seriesCount = seriesInformation.seriesData.length;
        }
        seriesInformation.seriesData.forEach((series, index) => {
          if (
            !settings[`seriesType_${index}`] ||
            settings[`seriesType_${index}`].label != `${series.name} graph type`
          ) {
            changed = true;
            settings[`seriesType_${index}`] = {
          ...
          let type = `area`;
          if (config[`seriesType_${index}`])
            type = config[`seriesType_${index}`];
          series.type = type;
        });

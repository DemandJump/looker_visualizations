looker.plugins.visualizations.add({
        id: 'DJ_Look_With_Graph',
        label: 'This is a single metric look with a graph connected to it',
        options: {},
        ...
        function maxAndMin() {
            data.forEach(node => {
                measures.forEach(mes => {
                    if (node[mes].value < min) min = node[mes].value;
                    if (node[mes].value > max) max = node[mes].value;
                });       
            });
            ...
            data.forEach(node => {
                ...
                dimensions.forEach( (dim, index) => {
                    if (index != dimensions.length - 1) {
                        node.chartName = node.chartName + node[dim].value + '-';
                    } else {
                        node.chartName = node.chartName + node[dim].value;
                    }
                });
